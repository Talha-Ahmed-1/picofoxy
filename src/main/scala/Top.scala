...
class Picofoxy(programFile: Option[String], val n:Int = 32) extends Module {
  val io = IO(new Bundle {
    val gpio_io = Vec(n, Analog(1.W))
    val rx_i = Input(UInt(1.W))
    // val CLK_PER_BIT = Input(UInt(16.W))
    // val done = Output(Bool())
    // val invdone = Output(Bool())
    // val puartValid = Output(Bool())
  })
  ...
  // io.done := top.io.done
  // io.invdone := top.io.invdone
  // io.puartValid := top.io.puartValid
}


class Top(programFile: Option[String], val n:Int = 32) extends Module {
  val io = IO(new Bundle {
  ...
  val imem = Module(new SRAM1kb(new WBRequest, new WBResponse)(programFile))
  ...
  // // io.invdone := ~puart.io.done
  // // io.puartValid := puart.io.valid
  // when(~puart.io.done){
